...
if __name__ == "__main__":
    ...
    ap.add_argument("--weights", type=str, help="path to JSON weights for improved eval")
    ...
    eng.move_ordering = "best-first"  # default for both algorithms
    if args.heuristic=="improved" and args.weights:
        with open(args.weights,"r",encoding="utf-8") as f:
            w=json.load(f)
        for k,v in w.items():
            if k in eng.theta: eng.theta[k]=float(v)

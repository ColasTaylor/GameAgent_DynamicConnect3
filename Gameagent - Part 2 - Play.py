...
class Agent:
    ...
    def run(self):
        ...
        while True:
            ...
            if not applied:
                # handshake/noise/illegal -> do NOT play; keep waiting
                continue
            if terminal:
                break
            # only play after a valid opponent move was applied
            self._play_our_turn()
        self.protocol.close()

    def _handle_incoming_line(self, line):
        """Decode and apply an incoming line from the server.
        ...
        # record position occurrence and check threefold
        try:
            if self.search.record_position_occurrence(self.state.tt_key()) >= 3:
                return (True, True)
        except Exception:
            pass

        tv = self.state.terminal_value() if hasattr(self.state, "terminal_value") else None
